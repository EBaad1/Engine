...
public abstract class WorldObject extends EngineObject {
    ...
    public void scale(double factor) {
        ...
        double xMin = Double.MAX_VALUE, xMax = Double.MIN_VALUE, yMin = Double.MAX_VALUE, yMax = Double.MIN_VALUE, tX, tY;
    ...
    public Bounds getBounds() {
        ...
        Primitive p;
        for (int i = 0; i < structure.size(); i++) {
            p = (Primitive) structure.get(i);
            for (int j = 0; j < p.vertices.size(); j++) {
                tX = p.vertices.get(j).x;
                tY = p.vertices.get(j).y;
